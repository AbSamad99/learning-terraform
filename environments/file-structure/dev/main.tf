...
terraform {
  ...
  backend "s3" {
    ...
    key            = "enviroment/file-structure/dev/terraform.tfstate"
    region         = "ca-central-1"
...
# Here we are directly defining the environment and are not relying on the 'terrafrom.workspace' variable
locals {
